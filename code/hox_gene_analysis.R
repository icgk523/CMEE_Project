...
data_frame_hox = lepidoptera_hox
data_frame_mass = mass 
phylo_tree = phylo


table <- data_frame_hox %>%
  group_by(Accession, Hox) %>%
  summarise(Count = n(), .groups = 'drop') %>%
  pivot_wider(names_from = Hox, values_from = Count, values_fill = list(Count = 0)) %>%
  ungroup() %>%
  mutate(hoxsums = rowSums(across(-Accession)))
...
for (hox_gene in hox_genes){
  formula <- as.formula(paste(hox_gene, "~ log10(Mean_Converted_Value) + (1 | Species)"))
  formula_sq <- as.formula(paste(hox_gene, "~ poly(log10(Mean_Converted_Value), 2, raw = TRUE) + (1 | Species)"))
  null_formula <- as.formula(paste(hox_gene, "~ 1 + (1 | Species)"))
  
  model_fitting_functions <- list(
    pglmmModel_Poisson = function() suppressMessages(pglmm(formula, data = data_frame, family = "poisson", cov_ranef = list(Species = phylo_tree), REML = FALSE)),
    ...
    pglmmModel_Binom = function() suppressMessages(pglmm(formula, data = data_frame, family = "binomial", cov_ranef = list(Species = phylo_tree), REML = FALSE)),
  ...
  # Calculate delta AIC relative to the null models
  delta_AICs <- list()
...
for (hox_gene in hox_genes){
  # pglmmModel_Poisson = pglmm(formula, data = data_frame, family = "poisson", cov_ranef = list(Species = phylo_tree), REML = FALSE)
  null_formula <- as.formula(paste(hox_gene, "~ 1 + (1 | Species)"))
  pglmmModel_Poisson_Null = pglmm(null_formula, data = data_frame, family = "poisson", cov_ranef = list(Species = phylo_tree), REML = FALSE)
  ...
  intercept <- pglmmModel_Poisson_Null$B[1]
  if (is.na(pglmmModel_Poisson_Null$B[2])){
  ...
  new_data$predicted <- exp(intercept + coef_log10_Mean_Converted_Value * new_data$log10_Mean_Converted_Value)
  
  plot_name <- paste0("plot_pglmm_", hox_gene)
  
  if (pglmmModel_Poisson_Null$B.pvalue[1] == 0 || pglmmModel_Poisson_Null$B.pvalue[1] < 0.001){
    p_text = "p < 0.001"
  } else {
    p_text = paste0("p = ", round(pglmmModel_Poisson_Null$B.pvalue[1], 3))
  }
  
  hox_gene_label = hox_gene
  if (hox_gene == "hoxsums"){
...
#### THIS IS FOR PLOTTING INSECTA FIGURES FOUND IN THE DISSERTATION - ONLY RUN THIS BELOW IF YOU HAVE RUN THE ABOVE ON INSECTA ####
...
remove(plot_pglmm_ShxA, plot_pglmm_ShxB, plot_pglmm_ShxC, plot_pglmm_ShxD, plot_pglmm_Ftz, plot_pglmm_Antp)
plot_pglmm_abd_A = plot_pglmm_abd_A + labs(y = "Abd-A")
plot_pglmm_hoxsums = plot_pglmm_hoxsums + labs(y = "Sum of Hox Genes") + theme(axis.text.y = element_text(angle = 90, vjust = 0.5, hjust = 0.5))
